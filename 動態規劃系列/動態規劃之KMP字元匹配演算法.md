...
KMP 算法（Knuth-Morris-Pratt 算法）是一個著名的字串匹配演算法，效率很高，但是確實有點複雜。

很多讀者抱怨 KMP 算法無法理解，這很正常，想到大學教材上關於 KMP 算法的講解，不知道有多少未來的 Knuth、Morris、Pratt 因為複雜的解釋被提前勸退了。有一些優秀的同學通過手推 KMP 算法的過程來輔助理解該算法，這是一種方式，不過本文要從邏輯層面幫助讀者理解算法的原理。十行代碼之間，KMP 灰飛煙滅。

**先在開頭約定，本文用 `pat` 表示模板字串，長度為 `M`，`txt` 表示全文字串，長度為 `N`。KMP 算法是在 `txt` 中查找子字串 `pat`，如果存在，返回這個子字串的起始索引，否則返回 -1**。

為什麽我認為 KMP 算法就是個動態規劃問題呢，等一下再解釋。對於動態規劃，之前多次強調了要明確 `dp` 陣列的含義，而且同一個問題可能有不止一種定義 `dp` 陣列意義的方法，不同的定義會有不同的解法。

讀者見過的 KMP 算法應該是，一波詭異的操作處理 `pat` 後形成一個一維陣列 `next`，然後根據這個陣列經過又一波複雜操作去匹配 `txt`。時間複雜度 O(N)，空間複雜度 O(M)。其實它這個 `next` 陣列就相當於 `dp` 陣列，其中元素的含義跟 `pat` 的前綴和後綴有關，判定規則比較複雜，不好理解。**本文則用一個二維的 `dp` 陣列（但空間複雜度還是 O(M)），重新定義其中元素的含義，使得程式碼長度大大減少，可讀性大大提高**。

PS：本文的代碼參考《算法4》，原程式碼使用的陣列名稱是 `dfa`（確定有限狀態機），因為我們的公眾號之前有一系列動態規劃的文章，就不說這麽高大上的名詞了，我對書中程式碼進行了一點修改，並沿用 `dp` 數組的名稱。
